...
module.exports = async (client, interaction, args) => {
    ...
    if (author.id == target.id) return client.errNormal({
        error: "Nem tagadhatod ki önmagad",
        type: 'editreply'
    }, interaction);

    if (target.bot) return client.errNormal({
        error: "Nem tagadhatsz ki egy botot",
        type: 'editreply'
    }, interaction);
    ...
    Schema.findOne({ Guild: interaction.guild.id, Parent: target.id }, async (err, data) => {
        if (data) {
            Schema.findOne({ Guild: interaction.guild.id, User: data.Parent }, async (err, data2) => {
                if (data2) {
                    client.embed({ title: `👪・Kitagadva`, desc: `${author} kitagadta őt: <@!${data.Parent}>`, type: 'editreply' }, interaction);
            ...
            Schema.findOne({ Guild: interaction.guild.id, User: author.id }, async (err, data) => {
                if (data) {
                    if (data.Children.includes(target.username)) {
                        ...
                        client.embed({ title: `👪・Kitagadva`, desc: `${author} kitagadta őt: <@!${target.id}>`, type: 'editreply' }, interaction);
                    }
                    else {
                        client.errNormal({ error: "Jelenleg nincsenek gyerekeid/szüleid", type: 'editreply' }, interaction);
                    }
                }
                else {
                    client.errNormal({ error: "Jelenleg nincsenek gyerekeid/szüleid", type: 'editreply' }, interaction);
