    ...
    Schema.findOne({ Guild: interaction.guild.id, Name: cmdname.toLowerCase() }, async (err, data) => {
        ...
        if (data) {
            Schema.findOneAndDelete({ Guild: interaction.guild.id, Name: cmdname.toLowerCase() }).then(async () => {
                ...
                if(!command) return client.errNormal({ error: "Nem található ez a parancs!", type: 'editreply' }, interaction );
                await interaction.guild.commands.delete(command.id);

                client.succNormal({
                    text: `A parancs sikeresen törölve`,
                    fields: [{
                        name: "🔧┆Parancs",
                        value: `\`\`\`${cmdname}\`\`\``,
                        inline: true,
            ...
            client.errNormal({ error: "Nem található ez a parancs!", type: 'editreply' }, interaction);
        }
    })
