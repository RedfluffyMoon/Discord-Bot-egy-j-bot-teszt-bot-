        ...
        if (data) {
            ...
            async function endGame() {
                if (player.score === 21) {
                    ...
                    gameOver = true;
                    endMsg(
                        `Nyertél! 21-et kaptál!`,
                        `A botnak ${dealer.score.toString()} volt`,
                        `GREEN`
                    );
                }
                if (player.score > 21) {
                ...
                if (dealer.score === 21) {
                    ...
                    gameOver = true;
                    endMsg(
                        `Vesztettél! A dealer 21-et kapott!`,
                        `A botnak ${dealer.score.toString()} volt`,
                        `RED`
                    );
                }
                ...
                if (
                    ...
                    gameOver = true;
                    endMsg(`Döntetlen!`, `A botnak ${dealer.score.toString()} volt`, `RED`);
                }
            }
            ...
            async function loop() {
                ...
                endMsg(
                    "Laphoz nyomd meg a `h`-t, megálláshoz az `s`-t",
                    `Sok szerencsét ;)`,
                    client.color
                );
                ...
                interaction.channel.awaitMessageComponent({ filter, max: 1, time: 1200000, errors: ["time"] })
                    ...
                    .catch(_ => {
                        interaction.channel.send("Vesztettél!!");
                        bet("lose");
                        return;
                    });
            }
